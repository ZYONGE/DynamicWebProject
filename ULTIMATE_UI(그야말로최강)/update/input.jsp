...
<%
  ...
  String bookImage = book != null ? book.getBookImage() : "";
%>
<!DOCTYPE html>
<html lang="ko">
<head>
...
<main class="container">
  <div class="breadcrumb">
    ...
    <span class="sep">›</span>
    <% if (!bookId.isEmpty()) { %>
      <a href="../select/detail.jsp?bookId=<%= bookId %>"><%= bookName %></a>
      <span class="sep">›</span>
    <% } %>
    <span>수정</span>
  </div>
  ...
  <div class="form-card">
    <form action="update.jsp" method="post">
      <div class="form-group">
        ...
        <p class="form-hint">도서 ID는 변경할 수 없습니다</p>
      </div>
      <div class="form-group">
        <label>도서명 <span class="required">*</span></label>
        <input type="text" name="bookName" value="<%= bookName %>" placeholder="도서명을 입력하세요" required>
      </div>
      <div class="form-group">
        <label>저자 <span class="required">*</span></label>
        <input type="text" name="author" value="<%= author %>" placeholder="저자명을 입력하세요" required>
      </div>
      <div class="form-group">
        <label>표지 이미지 파일명</label>
        <input type="text" name="bookImage" value="<%= bookImage %>" placeholder="예: book1.jpg">
      ...
      <div class="form-actions">
        ...
        <% if (!bookId.isEmpty()) { %>
          <a href="../select/detail.jsp?bookId=<%= bookId %>" class="btn btn-outline">취소</a>
        <% } else { %>
